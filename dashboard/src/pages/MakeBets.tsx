...
import { Card, Container, Paper, Box, Button, Grid, Typography } from '@material-ui/core';
...
function BetCard(props: { bet: Bet }) {
    ...
    return (
        <Paper>
            <div className={classes.image} style={{ backgroundImage: `url(${props.bet.imageUrl})` }}></div>
            ...
            <Box display="flex" flexDirection="row" my={4}>
                <Box flex={1} display="flex" flexDirection="column" alignItems="center" justifyContent="center">
                    ...
                    <Typography variant="body1" className={classes.positive}>
                        {(props.bet.expectedIrr * 100 - 100.0).toFixed(2)}%
                    </Typography>
                </Box>
                <Box flex={1} display="flex" flexDirection="column" alignItems="center" justifyContent="center">
        ...
        fetch('https://yolobets.herokuapp.com/yolobets').then((res) => {
            res.json().then((bets) => {
                setBets(
                    bets.map(
                        (bet: any) =>
                            new Bet(bet.title, bet.body, bet.amount, bet.expected_irr, bet.image_url, bet.taken)
                    )
                );
            });
        });
            ...
            <Box className={classes.wrapper}>
                ...
                <div className={classes.out}>
                    <Typography variant="body1">
                        That's all the bets for now. Come back later for some more investment opportunities!
                    </Typography>
                </div>
            </Box>
        </Box>
